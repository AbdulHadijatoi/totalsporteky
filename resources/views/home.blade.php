  ...
  @if (isset($setvar))
    ...
    <div class="table-container">
      <table>
        ...
        <tr class="dotted"><th ><a href="#" class="round-icon"><img src="{{ asset('storage/' . $allleague->image) }}" style="width: 58px; height: auto; object-fit: cover;" alt="Menu A"></a>  {{ $allleague->name }}</th></tr>
        @foreach ($allleague->match as $item)
        <tr class="dotted">
          <th >{{ $item->home_team_name}}</th>
          ...
          <th >{{ $item->away_team_name}}</th>
          {{-- <th>b{{ $item->id }}</th> --}}
          <th><a href="{{ route('detail-match',$item->id) }}" class="custom-button" >
            Live stream
          </a></th>
        </tr>
        @endforeach
        ...
        <tr class="dotted"><th ><a style href="#" class="round-icon"><img src="{{ asset('storage/' . $item->image) }}" style="width: 58px; height: auto; object-fit: cover;" alt="Menu A"></a>  {{ $item->name }}</th></tr>
        <tr class="dotted">
...
<th>{{ $items->away_team_name }}</th>
{{-- <th>a{{ $items->id }}</th> --}}
<th><a href="{{ route('detail-match',$items->id) }}" class="custom-button" >
  Live stream
</a></th>
</tr>
